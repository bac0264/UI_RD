...
namespace EnhancedScrollerDemos.NestedScrollers
{
    ...
    public class Controller : MonoBehaviour, IEnhancedScrollerDelegate
    {
        ...
        private void LoadData()
        {
            ...
            int j = 0;
            int m = mapManager.MapDataList().Length % RowCount;
            ...
            for (var i = 0; i < n; i++)
            {
                var masterData = new MasterData()
                {
                    ...
                    childData = new List<MapDataStat>()
                };
                _data.Add(masterData);
                for (; j < (RowCount * (i + 1)) && j < mapManager.MapDataList().Length; j++)
                {
                    masterData.childData.Add(mapManager.MapDataList()[j]);
                }
